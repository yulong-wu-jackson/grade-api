...
public class MongoGradeDB implements GradeDB {
    ...
    public Team getMyTeam() {
        ...
        try {
            ...
            if (responseBody.getInt("status_code") == 200) {
                JSONObject team = responseBody.getJSONObject("team");
                JSONArray memersArray = team.getJSONArray("members");
                String[] members = new String[memersArray.length()];
                for (int i = 0; i < memersArray.length(); i++) {
                    members[i] = memersArray.getString(i);
                }
